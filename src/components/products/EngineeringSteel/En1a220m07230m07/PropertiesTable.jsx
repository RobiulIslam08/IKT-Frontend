...
export default function PropertiesTable() {
  const chemicalData = [
    { grade: 'EN1A (230M07)', c: '0.07-0.15', si: '0.10 Max', mn: '0.80-1.20', p: '0.07 Max', s: '0.20-0.30' }
  ];

  const equivalentsData = [
    { bs1991: '220M07', bs1955: 'EN1A', aisi: '1113', werkstoff: '1.0711' },
    { bs1991: '230M07', bs1955: 'EN1A', aisi: '1213', werkstoff: '1.0715' }
  ];

  // Exact inventory sizes retained as per requirements
  const sizes = {
    ...
    <section className="py-16 md:py-24 px-4 sm:px-6 lg:px-8 bg-gray-50">
      <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }} className="max-w-7xl mx-auto">
        ...
        <motion.div variants={itemVariants} className="mb-12">
          ...
          <p className="text-base font-medium text-gray-600 ml-7">Complete data for EN1A Mild Steel (230M07 / 220M07)</p>
        </motion.div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {equivalentsData.map((row, index) => (
                    <tr key={index} className="border-b text-center font-bold text-primary hover:bg-gray-50">
                      <td className="px-4 py-3">{row.bs1991}</td><td className="px-4 py-3 text-gray-700 font-medium">{row.bs1955}</td><td className="px-4 py-3">{row.aisi}</td><td className="px-4 py-3">{row.werkstoff}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
              ...
              <div className="bg-gray-50 p-4 rounded-lg mb-6 border border-gray-100">
                 ...
                 <p className="text-sm text-gray-700 mb-1 font-semibold">• Up to 1" dia: +/- 0.00025"</p>
                 <p className="text-sm text-gray-700 mb-1 font-semibold">• 4mm to 30mm dia: +0mm / -0.0127mm</p>
                 <p className="text-sm text-gray-700 font-semibold">• 35mm to 50mm dia: +0mm / -0.0254mm</p>
              </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {sizes.imperialFlats.map((flat, idx) => (
                  <li key={idx} className="flex gap-4 text-sm bg-gray-50 p-2 rounded hover:bg-gray-100 transition-colors">
        ...
        <motion.div variants={itemVariants} className="mt-8 p-6 bg-primary/5 border border-primary/20 rounded-lg text-center">
          <p className="text-sm md:text-base font-medium text-gray-700">
            <span className="font-bold text-gray-900">Note from IKT:</span> Other specific sizes and lengths, including bespoke cut lengths, are available upon request. Please contact our technical sales team for precise stock availability and custom manufacturing solutions.
          </p>
        </motion.div>
