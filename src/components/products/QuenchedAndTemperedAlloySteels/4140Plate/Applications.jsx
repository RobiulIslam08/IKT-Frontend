...
export default function Applications() {
  const applications = [
    'Machinery structural parts and housings',
    'High-stress Shafts and Crankshafts',
    'Heavy-duty Gears and Pinions',
    'Drill Collars and Oil Patch fittings',
    'Aerospace structural components',
    'Industrial Slitters and Die holders',
    'Impact-resistant precision collars'
  ]
    ...
    <section className="relative py-20 px-4 sm:px-6 lg:px-8 bg-white overflow-x-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.4, duration: 0.5 }}>
              <button className="group relative px-6 py-3 bg-primary text-white font-semibold rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
                ...
                <span className="relative flex items-center gap-2 text-sm tracking-wider uppercase font-bold">
                  Request Pricing
                  <motion.svg
                    className="w-4 h-4"
                    fill="none"
          ...
          >
            <div className="w-full h-72 md:h-[380px] lg:h-[400px] xl:h-[430px] rounded-xl overflow-hidden shadow-2xl border border-gray-100">
              <img
                src={appImage}
                alt="AISI 4140 Steel Plate Applications"
                className="w-full h-full object-cover object-center"
              />
            </div>
            <div className="absolute -z-10 -bottom-6 -right-6 w-32 h-32 bg-primary/10 rounded-full blur-2xl"></div>
          </motion.div>
        </div>
      </div>
